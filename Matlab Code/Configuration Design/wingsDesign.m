function ME = wingsDesign(AC, ME, DP, Parameters, CST, CF) %#ok<INUSD>
%WINGSDESIGN Summary of this function goes here
...
%Load graph from NACA Technical Note 2751
    ...
    if EffectiveSweep1 < 30
        f_low   = interp1(f_00deg(:,1),f_00deg(:,2),AC.Wing1.eta,'linear','extrap'); %#ok<NODEF>
        f_hight = interp1(f_30deg(:,1),f_30deg(:,2),AC.Wing1.eta,'linear','extrap'); %#ok<NODEF>
        f1 = f_low+(f_hight-f_low)./30.*EffectiveSweep1;
    elseif EffectiveSweep1 < 45
        f_low   = interp1(f_30deg(:,1),f_30deg(:,2),AC.Wing1.eta,'linear','extrap'); %#ok<NODEF>
        f_hight = interp1(f_45deg(:,1),f_45deg(:,2),AC.Wing1.eta,'linear','extrap'); %#ok<NODEF>
        f1 = f_low+(f_hight-f_low)./15.*(EffectiveSweep1-30);
    elseif EffectiveSweep1 < 60
        f_low   = interp1(f_45deg(:,1),f_45deg(:,2),AC.Wing1.eta,'linear','extrap'); %#ok<NODEF>
        f_hight = interp1(f_60deg(:,1),f_60deg(:,2),AC.Wing1.eta,'linear','extrap'); %#ok<NODEF>
...
clear a T P rho nu EffectiveSweep1 EffectiveSweep2 CruiseMach1 CruiseMach2 tauWing
