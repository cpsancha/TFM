...
% Selection of wing configuration and geometric characteristics
    ...
    function F = getWings (x,AC, DP, ME, Parameters)

AC.Wing1.Incidence = x(1);
AC.Wing2.Incidence = x(2);
...
%x value in Fig E-5:
x = 2*pi*AC.Wing1.AspectRatio/(cl_alpha*cos(AC.Wing1.Sweep_14*pi/180)); %<--Te sobra un 2 al pasar a radianes, no? por qué no usas cosd() y así no te equivocas nunca?
...
CL0 = CL_alpha_wf *( (0 - alpha_0_1*AC.Wing1.Torsion)+ K_I/K_II*(AC.Wing1.Incidence - alpha_l0))+deltazCL; %Creo que esta mal, yo al menos no lo entiendo asi
...
AC.Wing2.CL_wf = CL_alpha_wf *( (0 - alpha_0_1*AC.Wing2.Torsion)+ K_I/K_II*(AC.Wing2.Incidence - alpha_l0))+deltazCL;
...
W = AC.Weight.MTOW*(CruiseFF + CruiseFF*Parameters.fuelFraction(5).value)/2;
...
F(2) = AC.Wing1.Cm_ac_wf + AC.Wing2.Cm_ac_wf*qh_q*Sh_S*ch_c ...
    ...
    - AC.Wing2.CL_wf*qh_q*Sh_S*(x_ac-x_cg)/AC.Wing1.CMA;
